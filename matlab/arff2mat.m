...
if (exist('weka2matlab.m','file') == 0),
    run('/home3c/luyi/Matlab/setup_arff.m');
end
...
matfile = sprintf('../data_all/%s/iemocap_%dcls_impro.mat', feaType, nClasses);
fprintf('Output file: %s\n', matfile);

% Label files (consider 'exc' as 'hap'
labfile = '../labels/emo_labels_impro.txt';

% Arff files
arffdir = sprintf('/home3c/luyi/iemocap_gan2/arff/%s',feaType);
labmap = containers.Map(labels,classes);
n_emos = length(classes);

% Read training data 
dat = get_iemocap_data(arffdir, labfile, labmap);
dat.classnames = labels;
