...
% Output:
%   Write CV dataset to datadir/iemocap_loso*.mat
% Example:
%   prepare_loso_cv('../data/IS09_emotion');
%   prepare_loso_cv('../data/IS09_emotion', 'iemocap_4cls.mat.mat');
%   prepare_loso_cv('../data/IS11_speaker_state', 'iemocap_4cls.mat.mat', []);
%   prepare_loso_cv('../data/IS11_speaker_state', 'iemocap_4cls.mat.mat', 500);
%   prepare_loso_cv('../data/IS11_speaker_state', 'iemocap_4cls.mat.mat', [], 1);
function prepare_loso_cv(datadir, datafile, nFeatures, n_ev)
addpath '~/so/Matlab/nap';

if ~exist('datafile','var')
    datafile = 'iemocap_4cls.mat';
end
...
P = eye(size(data.x,2));
spknums = data.spknums;
nSpks = length(unique(spknums));
for i = 1:nSpks,
    trnidx = find(spknums ~= i);
    trn = extract_data(data, trnidx);
    if n_ev > 0,
        [trn.x, P] = make_nap_data(trn.x, trn.targets, n_ev);
    end
    loso_cv_file1 = sprintf('%s/iemocap_trn_loso%d.mat',datadir,i);        
    ...
    tst.x = (P*tst.x')';
    loso_cv_file1 = sprintf('%s/iemocap_tst_loso%d.mat',datadir,i);
